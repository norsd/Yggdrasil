...
class myMdSpi(KSMdSpi):
    ...
    def __init__(self, instruments, broker_id,
                 investor_id, passwd,fnOnTick,**kwargs):
...
class myTraderSpi(KSTraderSpi):
	...
	res={"LoginTime":pRspUserLogin.LoginTime,"TradingDay":pRspUserLogin.TradingDay,"MaxOrderRef":pRspUserLogin.MaxOrderRef,"BrokerID":pRspUserLogin.BrokerID}
	db.UserLogin.insert(res)
	#insert to the database of the tradingRecord
	res={"TradingDay":pRspUserLogin.TradingDay,"OrderRef":pRspUserLogin.MaxOrderRef,"BrokerID":pRspUserLogin.BrokerID}
    ...
    def OnRspUserLogout(self, pUserLogout, pRspInfo, nRequestID, bIsLast):
        ...
        print repr(pUserLogout)
        if bIsLast and not self.isErrorRspInfo(pRspInfo):
            self.Req_UserLogin(self.broker_id,self.investor_id,passwd)
   ...
   #请求查询交易所
	...
	'''
        ...
        if pOrder.OrderStatus == KSUserApiType.THOST_FTDC_OST_NoTradeQueueing and int(str(pOrder.OrderSysID)) != 0:
	...
	connection=pymongo.Connection('localhost',27017)
	db=connection.commodityTrading
	res={"OrderRef":pTrade.OrderRef,"Price":pTrade.Price,"Volume":pTrade.Volume,"TradeTime":pTrade.TradeTime,"InstrumentID":pTrade.InstrumentID,"Direction":pTrade.Direction,"TradeDate":pTrade.TradeDate}
	db.RtnTrade.insert(res)	
...
inst = [u'IF1309', u'CF1305', u'ru1303', u'cu1303']
...
def ks_spi_init():
    ...
    spi = myTraderSpi(broker_id = broker_id,investor_id = investor_id,passwd = passwd,instrument_id = inst[0])
    ...
    #data=creatData(ins,orderRef,limitPrice,vol)    
    #user.ReqOrderInsert(data, requestid)    
