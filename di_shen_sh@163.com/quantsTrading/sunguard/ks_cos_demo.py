...
def ks_spi_init():
    ...
    #global spi
    spi = myTraderSpi(instruments = inst[0],broker_id = broker_id[0],investor_id = investor_id[0],passwd = passwd[0])
    user.RegisterSpi(spi)
    user.SubscribePublicTopic(2)
    user.SubscribePrivateTopic(2)
    user.RegisterFront(front[0]) 
    user.Init()
    time.sleep(3)
    cosspi = myCosSpi(broker_id = broker_id[0],investor_id = investor_id[0],passwd = passwd[0])
    cosapi = user.LoadExtApi(cosspi,"KSCosApi")

    #条件单录入请求
    req = KSUserApiStruct.CTKSConditionalOrderInitInsert()
    #print req
    req.BrokerID = broker_id[0]
    req.ExchangeID = "CFFEX"
    req.InvestorID = investor_id[0]
    req.InstrumentID = "IF1309"
    req.Direction = '0'
    ...
    #条件单查询请求
    req = KSUserApiStruct.CTKSConditionalOrderQuery(BrokerID=broker_id,InvestorID = investor_id)
    #print req
    requestid += 1
